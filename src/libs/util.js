...
util.date2Varchar = function(date) {
  ...
  if (util.getType(date) == "string") {
    if (date.length == 8) {
      ...
      return util.dateFormat(util.transform2Date(date, 'datestring'), "YYYYMMDD")
    } else {
      return ""
    }
  }
...
util.transform2Date = function (val = "", type) {
  ...
  if (type == "datestring") {
    return new Date(parseInt(val.slice(0, 4)), parseInt(val.slice(5, 7)) - 1, parseInt(val.slice(8, 10)))
  } else if (type == "varchar8") {
    return new Date(parseInt(val.slice(0, 4)), parseInt(val.slice(4, 6)) - 1, parseInt(val.slice(6, 8)))
  }
...
util.configTableCol = (colArr, config = {}) => {
  for (let i = 0, len = colArr.length; i < len; i++) {
    ...
    if (colArr[i].children) {
      // for (let j = 0; j < colArr[i].children.length; j++) {
      //   let item = colArr[i].children[j];
      //   if (!item.width) {
      //     item.minWidth = item.minWidth || 200;
      //   }
      // }
...
util.numFormat = (num, ifDecimal = false) => {
  let _num = +num;
  let res = _num.toString().replace(/\d+/, function(n){ // 先提取整数部分
...
util.forceGetData = (ele, arr) => {
  return new Promise((resolve, reject) => {
    ...
    let clock = setInterval(()=>{
      try{
        if (_this.$refs[arr[0]][arr[1]] != "" || _this.$refs[arr[0]][arr[1]].length > 0) {
          clearInterval(clock);
...
util.getParentEle = (ele, className) => {
  ...
  if (ele instanceof MouseEvent) {
    ...
    } else if (browserMess.includes("MSIE") || browserMess.includes("Windows NT")) {
 ...
 */
 util.resetTableColWidth = (fixHeadWidFront = [], fixHeadWidEnd = []) => {
  ...
  let ifOverLong = bodyTable.offsetHeight - bodyTable.parentElement.offsetHeight > 0;
  ...
  for (let i = tableWids.length - 1, len = tableWids.length - fixHeadWidEnd.length; i >= len; i--) {
